...
################################################################################
# Detector/Estimator implementations
################################################################################
...
class ExtendedBussgangDetector:
    '''
    Bussgang symbol detector for 1-bit channels, with
    Q(r) a component-wise sign operator
        r = Hx + z, y = Q(r)
...
class BussgangEstimatorMIMOChannel:
    '''
    ...
    def bzf_est(self, y_tsr, A_tsr, S_n_tsr):
        ...
        A_pinv_tsr = la.pinv(A_tsr)
        W_tsr = A_pinv_tsr
        W_tsr_tr = W_tsr.swapaxes(1,2)
        #x_hat_tsr = W_tsr @ y_tsr
        x_hat_tsr = W_tsr @ y_tsr_tr
    ...
    def estimate(self, y_tsr, x_tsr, n_var_tsr):
        '''
            estimate each row of channel separately
            using the same weight matrix

            Dimensions:
            y_tsr.shape = [N, N_rx, N_t]
            x_tsr.shape = [N, K   , N_t]
        '''
        ...
        # vectorized version of
        # X @ X.H + n_var * I
        X_herm = np.conj(X).swapaxes(1,2)
        S_r_tsr = X @ X_herm + n_var * I
        ...
        # estimate channel for each row
        # y_tsr.shape = [N, N_rx, N_t]

        # split observed
        h_hat_tsr, w_tsr = self.est(y_tsr, A_tsr, S_n_tsr)
